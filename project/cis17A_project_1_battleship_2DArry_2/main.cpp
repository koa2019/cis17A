 ...
 * Created:October 26, 2022,  7:43 PM
 * Purpose: Project 1 Covers chapters 9-11 in Gaddis. 
 * Version 1:  
 *           add Player structure. Initialize player->name by 
 *           calling getName() 
 */
...
void cGssArr(const char [][COL], const char [][COL],const int,const int,int, int); // confirms guess[][] filled correctly             
void chckArr(char &);
void copyAdd(string [],const int,string[],const int,string);
void fileSum(int,int,int,int);
void fllGArr(char [][COL],char [][COL],char [], int, int, int &, int&);
...
void runMenu(string, char &, char &, int,int,int,int,char [][COL],char [][COL],
             char [][COL],char [][COL],int,int,int,int,int &,int&,string [],int,
             string [],int,vector<string> &);
void pause();
string pickP2(string [],int);  // randomly picks a number for player 2
void plyrShpBrd(int,int,const char [][COL],const char [][COL],const int,const int);
void print2DArr(const char [][COL],const char [][COL],
                const char [][COL],const char [][COL],const int,const int,int &,int &);
void prntArr(string []);  // prints names[]
void prntArr(const char [][COL],int,int); //prints 2D
...
void upper(string &name){
    ...
    for(int i=0;i<name.length();i++){
        ch = toupper(name[i]);  // capitialize each letter
        temp +=ch;  //add each char to form a captializaed string
    }
    name=temp;
}
...
Player *getName(){ 
    string name1;
    Player *player=new Player;  // creates a single player
